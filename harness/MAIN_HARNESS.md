...
(실제 코드 기준: `backend/routers/analyze.py`, `backend/services/llm_service.py`)
...
요청 예시:
{
  ...
  "job_type": "데이터 분석",
  "experience_years": 0,
  "preferred_company_size": "무관"
}

응답 필드:
- answer (string)
- sources (array, 각 항목: company, title, required_skills, distance)

주의: `matched_skills`, `missing_skills`, `recommended_projects`, `confidence` 필드는
존재하지 않는다. 화면에서 이 필드들을 참조하지 않는다.
